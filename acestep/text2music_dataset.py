...
if __name__ == "__main__":
    ...
    with LYRIC_LOG_PATH.open("w", encoding="utf-8") as lyric_f, \
         WAV_LOG_PATH.open("w",   encoding="utf-8") as wav_f:
        lyric_over_cnt = 0
        wav_over_cnt   = 0
        ...
        for idx in (len(dataset)):
            ...
            if lyric_len > MAX_LYRIC_LEN:
                lyric_over_cnt += 1
                print(f"[LYRIC]  {key} ({lyric_len} tokens) > {MAX_LYRIC_LEN}")
                lyric_f.write(f"{key}\t{lyric_len}\n")

            if wav_len > MAX_WAV_LEN:
                wav_over_cnt += 1
                print(f"[AUDIO]  {key} ({wav_len} samples) > {MAX_WAV_LEN}")
                wav_f.write(f"{key}\t{wav_len}\n")

    print(f"\n--- Done ---")
    print(f"  가사 길이 초과 트랙  : {lyric_over_cnt:>4}개 (→ {LYRIC_LOG_PATH})")
    print(f"  음원 길이 초과 트랙 : {wav_over_cnt:>4}개 (→ {WAV_LOG_PATH})")