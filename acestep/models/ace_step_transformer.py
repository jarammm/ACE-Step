...
class PatchEmbed(nn.Module):
    ...
    def forward(self, latent):
        ...
        latent = self.early_conv_layers(latent) # [bs * 2, 8, 16, time_frames] -> [bs * 2, 2560, 1, time_frames]
        latent = latent.flatten(2).transpose(1, 2)  # BCHW -> BNC / [bs * 2, 2560, 1, time_frames] -> [bs * 2, time_frames, 2560]
        return latent
...
class ACEStepTransformer2DModel(
    ...
    def forward_lyric_encoder(
        self,
        lyric_token_idx: Optional[torch.LongTensor] = None, # [bs * 2, lyric_seq_len]
        lyric_mask: Optional[torch.LongTensor] = None, # [bs * 2, lyric_seq_len]
    ):
        # N x T x D
        lyric_embs = self.lyric_embs(lyric_token_idx) # [bs * 2, lyric_seq_len, 1024]
        prompt_prenet_out, _mask = self.lyric_encoder(
            lyric_embs, lyric_mask, decoding_chunk_size=1, num_decoding_left_chunks=-1
        ) # prompt_prenet_out : torch.Size([bs * 2, lyric_seq_len, 1024])
        prompt_prenet_out = self.lyric_proj(prompt_prenet_out) # [bs * 2, lyric_seq_len, 2560]
        return prompt_prenet_out

    def encode(
        self,
        encoder_text_hidden_states: Optional[torch.Tensor] = None, # [bs * 2, prompt_seq_len, 768]
        text_attention_mask: Optional[torch.LongTensor] = None, # [bs * 2, prompt_seq_len]
        speaker_embeds: Optional[torch.FloatTensor] = None, # [bs * 2, 512]
        lyric_token_idx: Optional[torch.LongTensor] = None, # [bs * 2, lyric_seq_len]
        lyric_mask: Optional[torch.LongTensor] = None, # [bs * 2, lyric_seq_len]
    ):

        bs = encoder_text_hidden_states.shape[0] # bs(학습 때 설정한 batch size) * 2
        device = encoder_text_hidden_states.device

        # speaker embedding
        encoder_spk_hidden_states = self.speaker_embedder(speaker_embeds).unsqueeze(1) # [bs * 2, 1, 2560]
        speaker_mask = torch.ones(bs, 1, device=device) # [bs * 2, 1]

        # genre embedding
        encoder_text_hidden_states = self.genre_embedder(encoder_text_hidden_states) # [bs * 2, genre_seq_len, 2560]

        # lyric
        # self.forward_lyric_encoder : embed -> conformer -> proj
        encoder_lyric_hidden_states = self.forward_lyric_encoder(
            lyric_token_idx=lyric_token_idx, # [bs * 2, lyric_seq_len]
            lyric_mask=lyric_mask, # [bs * 2, lyric_seq_len]
        ) # [bs * 2, lyric_seq_len, 2560]
        ...
        encoder_hidden_states = torch.cat(
            ...
            dim=1,
        ) # [bs * 2, lyric_seq_len + genre_seq_len + 1(speaker), 2560]
        encoder_hidden_mask = torch.cat(
            [speaker_mask, text_attention_mask, lyric_mask], dim=1
        ) # [bs * 2, lyric_seq_len + genre_seq_len + 1(speaker)]
    ...
    def decode(
        ...
        hidden_states = self.proj_in(hidden_states) # patch embed : # [bs * 2, 8, 16, time_frames] -> [bs * 2, time_frames, 2560]
    ...
    def forward(
        self,
        hidden_states: torch.Tensor, # [bs * 2, 8, 16, t]
        attention_mask: torch.Tensor,
