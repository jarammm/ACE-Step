...
class MusicDCAE(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def forward_mel(self, audios): # audios [bs, 2, wav(4096x)]
        mels = []
        for i in range(len(audios)):
            image = self.vocoder.mel_transform(audios[i])
            mels.append(image)
        mels = torch.stack(mels)
        return mels # [bs, 2, 128, audios.shape[-1]/512]
    ...
    def encode(self, audios, audio_lengths=None, sr=None):
        ...
        if max_audio_len % (8 * 512) != 0: # 8 * 512 = 4096
            audio = torch.nn.functional.pad(
                audio, (0, 8 * 512 - max_audio_len % (8 * 512))
            ) # 4096단위로 길이 맞추기 위해 패딩 / [bs, 2, 4096x]
        # 2. get log melspectrogram
        mels = self.forward_mel(audio) # [bs, 2, 128, audio.shape[-1]/512]
        mels = (mels - self.min_mel_value) / (self.max_mel_value - self.min_mel_value) # minmax scaling
        mels = self.transform(mels) # -1 ~ 1 scaling
        latents = []
        for mel in mels:
            latent = self.dcae.encoder(mel.unsqueeze(0)) # [8, 16, mel.shape[-1]/8]
            latents.append(latent)
        latents = torch.cat(latents, dim=0) # [bs, 8, 16, mel.shape[-1]/8]
        latent_lengths = (
            audio_lengths / sr * 44100 / 512 / self.time_dimention_multiple
        ).long()
        latents = (latents - self.shift_factor) * self.scale_factor # normalize with fixed shift & scale factor
